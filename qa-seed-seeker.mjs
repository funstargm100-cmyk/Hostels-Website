...
if (ex.rows.length) {
  res = await c.query(
    'UPDATE users SET password_hash=$1, role=\'seeker\', is_verified=true, base_location=\'Kathmandu, Nepal\', base_lat=27.7172, base_lng=85.3240 WHERE email=$2 RETURNING id, email, role',
    [hash, 'seeker@test.com']
  );
} else {
  res = await c.query(
    `INSERT INTO users (name, email, phone, password_hash, role, is_verified, base_location, base_lat, base_lng)
     VALUES ($1,$2,$3,$4,'seeker',true,'Kathmandu, Nepal',27.7172,85.3240)
