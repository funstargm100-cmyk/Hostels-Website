...
router.post('/signup', async (req, res) => {
  ...
  try {
    ...
    if (existing.rows.length) return res.status(409).json({ error: 'Account already exists with this email or phone' });
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    await sendEmail(email, 'Reset your password', `<p>Click <a href="${link}">here</a> to reset your password. Expires in 1 hour.</p>`);
