...
async function suSubmit(btnId, fields) {
  ...
  try {
    ...
    setTimeout(() => {
      // Pass sent=0 when signup could not deliver the code, so the login page knows
      // it must auto-send one. When the email went out, the login page stays quiet —
      // otherwise the user receives TWO codes and the first one stops working.
      location.href = '/login?verify=1&uuid=' + encodeURIComponent(res.uuid) +
        (emailQ ? '&email=' + encodeURIComponent(emailQ) : '') +
        (res.emailSent === false ? '&sent=0' : '') +
        '&t=' + Date.now();
    }, 1200);
  } catch (ex) {
