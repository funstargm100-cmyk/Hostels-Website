// QA: per-person pricing rules (agent per-occupant commission; owner 7%).
...
const email = 'pp2_' + stamp + '@example.com';
...
export default async function run(page) {
  ...
  const o = await db.query(
    ...
    "VALUES ('PP2 Owner',$1,$2,$3,'owner',TRUE,$4) RETURNING id",
    [email, '091' + String(stamp).slice(-7), hash, v4()]
  );
  ...
  try {
    ...
    await page.fill('#adTitle', 'QA PP2 Room');
    await page.fill('#adDescription', 'QA description for per-person pricing rules.');
    await page.click('#nextBtn'); // -> step 3

    await page.selectOption('#adOccupancy', '2'); // 2-in-1
    await page.fill('#adOriginalPrice', '1000');  // P = 1000 per person
    ...
    const read = () => page.evaluate(() => {
      ...
      return {
        ...
        commissionTypeIsHiddenInput: document.getElementById('adCommissionType').type === 'hidden',
        ownerTotal: vis('prevRoomTotal') ? document.getElementById('prevRoomTotal').textContent : null,
        commission: vis('prevCommission') ? document.getElementById('prevCommission').textContent : null,
        commissionPerOcc: vis('prevTaxable') ? document.getElementById('prevTaxable').textContent : null,
    ...
    out.owner = await read();           // expect totalForOcc 2000, fee 70, perHead 1070

    await page.click('.poster-type-opt:has(input[value="agent"])');
    await page.waitForTimeout(150);
    await page.fill('#adCommission', '500');  // C = 500 per occupant
    await page.waitForTimeout(200);
    out.agent500 = await read();        // totalComm 1000, fee 100, perHead 1600

    await page.fill('#adCommission', '200');  // C = 200 per occupant
    await page.waitForTimeout(200);
    out.agent200 = await read();        // totalComm 400, fee 70, perHead 1270

    // Submit (agent, C=200) and verify persisted values.
    ...
    if (out.uuid) {
      const r = await db.query(
        'SELECT price_per_head, original_price, listed_price, platform_fee, commission_value, commission_type FROM listings WHERE uuid=$1',
        [out.uuid]
      );
